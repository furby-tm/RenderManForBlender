...
class addon_updater_update_now(bpy.types.Operator):
	...
	bl_description = "Update to the latest verison of the Renderman for Blender addon".format(
														x=updater.addon)
...
def update_settings_ui(self, context):
	...
	if updater.manual_only == False:
		...
		backuptext = "Restore addon backup (Renderman for Blender)".format(x=lastdate)
